...
void setup() {
  ...
  lcd.clear(); // Clear the display - this moves the cursor to home position as
               // well
  lcd.print("Hello, World!");
  myservo.attach(
      9); // tell the servo object that its servo is plugged into pin 9
  servoPosition = 0;
}
...
void loop() {
  ...
  potPosition = ((millis() / 100) % 180);
  lcd.println(potPosition);
  ...
  // avoid driving the little SIK servos all the way to 0 or 180 degrees it can
